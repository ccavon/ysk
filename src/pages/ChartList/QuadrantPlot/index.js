/**
 * @Author: chengyafang 
 * @Date: 2020-01-09 16:24:02 
 * @Last Modified by: chengyafang
 * @Last Modified time: 2020-01-13 11:51:38
 * @file 根据BizChart官网里面的 #带文本的气泡图# 为基础将修改为 四维象限图
 */
import React, { PureComponent } from 'react';
import PageHeaderWrapper from '@/components/PageHeaderWrapper';
import { Chart, Axis, Tooltip, Geom, Label, Guide } from 'bizcharts';

class QuadrantPlot extends PureComponent {
  render() {
    const data = [
      {
        x: 95,
        y: 95,
        z: 13.8,
        name: "BE",
        country: "Belgium"
      },
      {
        x: 86.5,
        y: 102.9,
        z: 14.7,
        name: "DE",
        country: "Germany"
      },
      {
        x: 80.8,
        y: 91.5,
        z: 15.8,
        name: "FI",
        country: "Finland"
      },
      {
        x: 80.4,
        y: 102.5,
        z: 12,
        name: "NL",
        country: "Netherlands"
      },
      {
        x: 80.3,
        y: 86.1,
        z: 11.8,
        name: "SE",
        country: "Sweden"
      },
      {
        x: 78.4,
        y: 70.1,
        z: 16.6,
        name: "ES",
        country: "Spain"
      },
      {
        x: 44.2,
        y: 68.5,
        z: 14.5,
        name: "FR",
        country: "France"
      },
      {
        x: 73.5,
        y: 83.1,
        z: 10,
        name: "NO",
        country: "Norway"
      },
      {
        x: 71,
        y: 93.2,
        z: 24.7,
        name: "UK",
        country: "United Kingdom"
      },
      {
        x: 59.2,
        y: 57.6,
        z: 10.4,
        name: "IT",
        country: "Italy"
      },
      {
        x: 68.6,
        y: 20,
        z: 16,
        name: "RU",
        country: "Russia"
      },
      {
        x: 45.5,
        y: 126.4,
        z: 35.3,
        name: "US",
        country: "United States"
      },
      {
        x: 25.4,
        y: 50.8,
        z: 28.5,
        name: "HU",
        country: "Hungary"
      },
      {
        x: 33.4,
        y: 51.8,
        z: 15.4,
        name: "PT",
        country: "Portugal"
      },
      {
        x: 14,
        y: 82.9,
        z: 31.3,
        name: "NZ",
        country: "New Zealand"
      }
    ];
    const cols = {
      x: {
        alias: "Daily fat intake", // 定义别名
        tickInterval: 10, // 自定义刻度间距
        nice: false, // 不对最大最小值优化
        max: 100, // 自定义最大值
        min: 0 // 自定义最小值
      },
      y: {
        alias: "Daily sugar intake",
        tickInterval: 20,
        nice: false,
        max: 165,
        min: 0
      },
      z: {
        alias: "Obesity(adults) %"
      }
    };
    return (
      <PageHeaderWrapper title={'四维象限图'}>
        <Chart
          height={600}
          data={data}
          padding={[20, 0, 80, 80]}
          scale={cols}
          forceFit
          plotBackground={{ // 图表绘图区域的边框和背景样式，是一个对象
            stroke: '#ccc', // 图表边框颜色
            lineWidth: 1,   // 图表边框粗度
            // lineDash: [0, 0, 0, 2]   // 虚线的设置
          }}
        >
          <Axis
            name={'x'}
            label={{
              formatter: val => {
                return `${val}gr`; // 格式化坐标轴显示文本
              }
            }}
          // grid={{               // 网格线
          //   lineStyle: {
          //     stroke: "#d9d9d9",// 网格线的颜色
          //     lineWidth: 1,     // 网格线的宽度复制代码
          //     lineDash: [2, 2]  // 网格线的虚线配置，第一个参数描述虚线的实部占多少像素，第二个参数描述虚线的虚部占多少像素
          //   }
          // }}
          />
          <Axis
            name={'y'}
            title={{ offset: 64 }}
            label={{
              formatter: val => {
                if (val => 0) {
                  return `${val}gr`;
                }
              }
            }}
          />
          <Tooltip title={'country'} />
          <Geom
            type={'point'}
            position={'x*y'}
            color="#1890ff"
            // 气泡的边框线的颜色
            // style={{
            //   lineWidth: 1,
            //   stroke: "#1890ff"
            // }}
            shape={'circle'}
            // size={['z', [10, 40]]}
            tooltip={"x*y*z"}
          // opacity={0.3}
          >
            {/* 有无气泡 */}
            <Label
            // content={"name*country"}
            // offset={0}
            // textStyle={{ fill: "#1890FF" }}
            />
          </Geom>
          <Guide>
            <Guide.Line
              start={["min", 20]}
              end={["max", 20]}
              text={{
                content: "Safe sugar intake 50g/day",
                position: "end",
                style: {
                  textAlign: 'end'
                }
              }}
              // 图形样式配置
              lineStyle={{
                stroke: '#000', // 线的颜色
                lineDash: [0], // 虚线的设置
                lineWidth: 1 // 线的宽度
              }}
            />
            <Guide.Line
              start={[10, "min"]}
              end={[10, "max"]}
              text={{
                content: "Safe fat intake 65g/day",
                position: "end",
                autoRotate: false,
                style: {
                  textAlign: 'start'
                }
              }}
              // 图形样式配置
              lineStyle={{
                stroke: '#000', // 线的颜色
                lineDash: [0], // 虚线的设置
                lineWidth: 1 // 线的宽度
              }}
            />
          </Guide>
        </Chart>
      </PageHeaderWrapper>
    )
  }
}
export default QuadrantPlot;